...
from agents.skeptic import SkepticAgent, SkepticReport
...
class JudgeFactory:
    ...
    def _judge_enricher(output: Dict, context: Dict) -> JudgeResult:
        ...
        for field_name in ["instagram", "website", "email", "cnpj"]:
            if field_name in str(dossier).lower() and field_name not in provided_fields:
                hallucination_score -= 5
                explanations.append(f"Possível alucinação: '{field_name}' mencionado mas não fornecido no input")
        ...
        for dim in report.dimension_results:
            if not dim.passed:
                if dim.dimension == Dimension.ABDUCTIVE_RESILIENCE:
                    ...
                    recommendations.append("[P3] Revisar arquitetura — possível god class ou duplicação")
