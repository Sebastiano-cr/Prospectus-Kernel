...
def test_register_locale():
    ...
    l = get_locale("mock")
    assert l.language_code == "mock"


def test_field_name():
    l = get_locale("pt-BR")
    assert l.get_field_name("profile_summary") == "resumo_perfil"
    assert l.get_field_name("weaknesses") == "pontos_fracos"
    assert l.get_field_name("nonexistent") == "nonexistent"


def test_field_names_batch():
    l = get_locale("pt-BR")
    result = l.get_field_names(["profile_summary", "weaknesses"])
...
def test_score_categories():
    l = get_locale("pt-BR")
    assert l.get_score_category(72) == "quente"
    assert l.get_score_category(45) == "morno"
    assert l.get_score_category(15) == "frio"
    assert l.get_score_category(100) == "quente"
    assert l.get_score_category(0) == "frio"


def test_score_ranges():
    l = get_locale("pt-BR")
    ranges = l.get_score_ranges()
...
def test_status_labels():
    l = get_locale("pt-BR")
    assert l.get_status_label("new") == "novo"
    assert l.get_status_label("enriched") == "enriquecido"
    assert l.get_status_label("message_sent") == "mensagem_enviada"


def test_fallbacks():
    l = get_locale("pt-BR")
    assert l.get_fallback("unknown_establishment") == "Estabelecimento desconhecido"
    assert l.get_fallback("opt_out_message") == "\n\nResponda SAIR para não receber mais contatos."
    assert l.get_fallback("nonexistent") == "nonexistent"


def test_json_only_suffix():
    l = get_locale("pt-BR")
    suffix = l.get_json_only_suffix()
...
def test_markers():
    l = get_locale("pt-BR")
    assert len(l.get_markers("generic_phrases")) >= 3
    assert len(l.get_markers("social_absence")) >= 3
    assert "raiva" in l.get_markers("emotion_anger")
    unknown = l.get_markers("unknown_type")
    assert unknown == []


def test_language_markers():
    l = get_locale("pt-BR")
    markers = l.get_language_markers()
...
def test_get_prompt_enricher():
    ...
    prompt = l.get_prompt("enricher",
        ...
        json_only_suffix=l.get_json_only_suffix(),
    )
...
def test_get_prompt_unknown():
    ...
    with pytest.raises(PromptNotFoundError):
        l.get_prompt("nonexistent_prompt_type")
